...
Every monster in `game/world_dungeon.tscn` is four pieces: a sprite, a brain,
a health node and a hurtbox. Select one in the editor and tune it in the
Inspector: `max_health` on Health, `chase_speed` and `detect_radius` on Brain,
`damage` on Touch. Duplicate a monster, give it a new `enemy_id`, and add that
id to the quest to make it count.
...
Twelve more systems are in the project already, switched off. Open Project
